...
const ActiveCollaborationTab = lazy(() => import("@/components/brand/ActiveCollaborationTab"));
const PendingCollaborationTab = lazy(() => import("@/components/brand/PendingCollaborationTab"));
const CompletedCollaborationTab = lazy(() => import("@/components/brand/CompletedCollaborationTab"));
...
export default function Page() {
  ...
  const VALID_TABS = ["Waiting Approval", "active", "completed"];
  ...
  const TabInfo = ({ text }: { text: string }) => (
    <span className="relative inline-flex items-center ml-0.5">
      <span className="group relative">
        ...
        />
        <span className="invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 absolute z-[100] w-56 sm:w-64 rounded-lg bg-primary text-primary-foreground shadow hover:bg-primary/90 px-3 py-2.5 text-xs leading-relaxed bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-normal">
          {text}
        </span>
      </span>
    </span>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="dark:bg-background h-full">
        ...
        <div className="overflow-visible sticky top-0 z-10 bg-background pb-2">
          <TabsList className="w-full !overflow-visible relative">
            <TabsTrigger value="Waiting Approval" className="relative h-12 sm:h-16 w-1/2 xs:w-1/4 text-xs md:text-sm overflow-visible">
            ...
            <TabsTrigger value="active" className="relative h-12 sm:h-16 w-1/2 xs:w-1/4 text-xs md:text-sm overflow-visible">
              <span className="flex items-center gap-1.5">
            ...
            <TabsTrigger value="completed" className="relative h-12 sm:h-16 w-1/2 xs:w-1/4 text-xs md:text-sm overflow-visible">
              <span className="flex items-center gap-1.5">
