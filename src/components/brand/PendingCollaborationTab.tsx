  ...
  useEffect(() => {
    const fetchPendingCollaborations = async () => {
      try {
        setIsLoading(true);
        if (user && user._id) {
          const response = await postApi.getPendingCollaboration(user._id);
          setCampaigns(response.campaigns || []);
        }
      } catch (err) {
        setError(err as Error);
        console.error("Error fetching pending collaborations:", err);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="h-[75vh] flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-600">Loading pending collaborations...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-10">
        <p className="text-red-500">Error loading pending collaborations</p>
      </div>
    );
  }
