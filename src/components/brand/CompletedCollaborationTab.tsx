...
export default function CompletedCollaborationTab() {
  ...
  useEffect(() => {
    const fetchCompletedCollaborations = async () => {
      try {
        setIsLoading(true);
        if (user && user._id) {
          const response = await postApi.getCompletedCollaboration(user._id);
          setCampaigns(response.campaigns || []);
        }
      } catch (err) {
        setError(err as Error);
        console.error("Error fetching completed collaborations:", err);
      } finally {
        setIsLoading(false);
      }
    };

    fetchCompletedCollaborations();
  }, [user]);
  ...
  if (error) {
    return (
      <div className="text-center py-10">
        <p className="text-red-500">Error loading completed collaborations</p>
      </div>
    );
  }
