export const blogs = [
  {
    image: "/landing/blog/newblog1.png",
    date: "23 Nov 24",
    title:
      "We caught up with the brilliant and insightful Sandeep Asdhir a few weeks ago and have shared our conversation below.",
    // description:
    //   "Hi Sandeep , appreciate you sitting with us today to share your wisdom with our readers. So, let’s start with resilience – where do you get your resilience from?My resilience comes from a mix of influences and personal practices that keep me grounded and motivated. First and foremost, I’ve constantly been inspired by watching my family work hard. To this day, my parents show me the power of perseverance, constantly pushing forward and embracing new challenges. There are many times when I visit my parents I find them to be awake and sleep after me. No complaints and doing the basics. Sometimes showing up is half the battle.",
    link: "https://boldjourney.com/meet-sandeep-asdhir/",
  },
  {
    image: "/landing/blog/newblog2.jpeg",
    date: "25 Oct 2024",
    title: "A Introduction To Influencer Marketing Part One",
    // description:
    //   "In today’s digital landscape, connecting with influencers is a powerful strategy for brands looking to enhance their visibility and engagement. However, reaching out to influencers effectively requires a thoughtful approach that prioritizes personalization, relationship-building, and clear communication. At Influenergy, we’ve developed a unique method for connecting brands with global content creators. Here’s how you can do the same, along with insights into our process.",
    // slug:"how-we-find-influencers",
    link:"https://www.influenergy.co/blog/what-is-influencer-marketing"
  },
  {
    image: "/landing/blog/newblog3.jpeg",
    date: "25 Oct 2024",
    title: "How Influencers Can Help Market Brands?",
    // description:
    //   "SO you know Kim K, Mr. Beast —yes, they’re influencers. Today, many people both revere and criticize this label. But what does it really mean to be an influencer?",
    // slug:"a-introduction-to-influencer-marketing-part-one",
    link:"https://www.influenergy.co/blog/how-influencers-can-help-market-brands"
  },
  {
    image: "/landing/blog/newblog4.jpg",
    date: "18 Oct 2024",
    title: "Why Influenergy Pivoted to an AI-Focused Marketing Agency",
    // description:
      // "Ever scroll through Instagram and see an Influencer in a homemade style video going on about a product. The enthusiasm is cranked up for the latest makeup must-have, or a travel destination that looks straight out of a dream? Chances are, that scripted review is part of a powerful marketing strategy called Influencer marketing.",
    // slug:"how-influencers-can-help-market-brands",
    link:"https://www.influenergy.co/blog/blog-post-title-two-na5me"
  },
  {
    image: "/landing/blog/newblog5.jpeg",
    date: "11 Oct 2024",
    title: "FEATURE: Exploring Life &amp; Business with Sandeep Asdhir of Influenergy Marketing",
    // description:
      // "The marketing landscape is evolving at an rapid pace, and to stay ahead of the curve, we knew we had to embrace the power of AI. It wasn't just about jumping on the latest trend; it was about recognizing the immense potential of AI to transform how we connect with audiences and achieve marketing goals.",
    // slug:"https://voyagela.com/interview/exploring-life-business-with-sandeep-asdhir-of-influenergy-marketing",
    link:"https://voyagela.com/interview/exploring-life-business-with-sandeep-asdhir-of-influenergy-marketing"
  },
  // {
  //   image: "/landing/blog/blog3.png",
  //   date: "30 Sep 2024",
  //   title: "FEATURE: Exploring Life & Business with Sandeep Asdhir of Influenergy Marketing ",
  //   description:
  //     "Enjoy the Feature I had with VoyageLA Magazine highlighting myself and Influenergy.",
  //   link:"https://voyagela.com/interview/exploring-life-business-with-sandeep-asdhir-of-influenergy-marketing"
  // },
];


// export const BlogContent =[
    
// ]